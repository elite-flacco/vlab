...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside
      className={`h-full flex flex-col bg-background border-r border-foreground-dim/20 transition-all duration-300 ${
        isCollapsed ? "w-16" : "w-64"
      }`}
    >
      <div
        className={`p-4 space-y-2 flex-1 overflow-y-auto overflow-x-hidden w-full ${isCollapsed ? "flex flex-col items-center" : ""}`}
      >
        {/* Community Hub */}
        <button
          ...
          className={`${isOnCommunity ? "sidebar-item-active" : "sidebar-item"} ${
            isCollapsed
              ...
              : "flex items-center space-x-2 text-sm font-semibold text-foreground hover:text-foreground transition-colors mb-3 px-3 py-2 rounded-md w-full text-left"
          }`}
          title="Community Hub"
        >
        ...
        <div
          className={`border-b border-foreground-dim/20 ${isCollapsed ? "my-3 w-6" : "my-4"}`}
        ></div>

        {/* Active Projects */}
        <div
          className={isCollapsed ? "flex flex-col items-center space-y-2" : ""}
        >
          {!isCollapsed && (
          ...
          <div className={`sidebar-section ${isCollapsed ? "space-y-2" : ""}`}>
            ...
            {loading && activeProjects.length === 0 && (
              <div
                className={`flex items-center ${isCollapsed ? "justify-center py-2" : "space-x-2 px-3 py-2"} text-foreground-dim`}
              >
            ...
            {error && activeProjects.length === 0 && !loading && (
              <div
                className={`${isCollapsed ? "flex flex-col items-center space-y-1" : "px-3 py-2"}`}
              >
                <div
                  className={`text-xs text-destructive ${isCollapsed ? "text-center" : ""}`}
                >
            ...
            {activeProjects.map((project) => (
              <button
                ...
                className={`${currentProject?.id === project.id && !isOnCommunity && !isOnScratchpad && !isOnSettings ? "sidebar-item-active" : "sidebar-item"} ${
                  isCollapsed
                    ? "w-8 h-8 p-2 flex items-center justify-center"
                    : ""
                }`}
